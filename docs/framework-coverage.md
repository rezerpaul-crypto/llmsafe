...
| --- | --- | --- | --- | --- | --- |
| OpenAI Agents SDK | Model output or a function-tool argument reaches a shell, code, URL, query, or dynamic callable | [`function_tool`, `@tool`, and injected context](https://github.com/openai/openai-agents-python/blob/cc1493686e957bba1cc58e4c344f12dfc251cc75/docs/tools.md#function-tools) | `Runner.run()`, `run_sync()`, and `run_streamed()` results are model sources. Ordinary parameters of official `@function_tool`/`@tool` functions are model-input sources; SDK-injected `RunContextWrapper` and `ToolContext` parameters are excluded. | Model text remains data; tool arguments are validated or mapped to application-owned operations; injected context is not treated as model input. | [`openai_agents_vulnerable.py`](../benchmarks/cases/frameworks/openai_agents_vulnerable.py) / [`openai_agents_safe.py`](../benchmarks/cases/frameworks/openai_agents_safe.py) |
...
| MCP Python SDK v1/v2 | An MCP tool exposes process, filesystem, network, or dynamic execution to a remote caller | [`FastMCP` to `MCPServer` migration and unchanged `@mcp.tool()` boundary](https://github.com/modelcontextprotocol/python-sdk/blob/6affe5c0d3588fd1705713b3703dc68015cfe3eb/docs/whats-new.md#fastmcp-is-now-mcpserver) | Parameters of tools registered on a statically resolved official `FastMCP`/`MCPServer` instance are remote-input sources. SDK-injected `Context` and `Resolve(...)` parameters are excluded. MCP JSON checks shell launch, plaintext remote transport, and wildcard grants. | Expose a fixed tool surface with validated arguments and fixed subprocess arguments. | [`mcp_vulnerable.py`](../benchmarks/cases/frameworks/mcp_vulnerable.py) / [`mcp_safe.py`](../benchmarks/cases/frameworks/mcp_safe.py) |
